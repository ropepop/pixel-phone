...
- Ticket start/stop serialization must inspect lock-owner command lines through a one-second bounded reader. A confirmed Ticket lifecycle owner retains the lock; a live owner whose command line cannot be read also retains it conservatively; dead owners and live PID reuse may be removed only after the owner file is read unchanged a second time. Cleanup remains limited to `owner.pid` plus the empty Ticket lock directory.
...
- 2026-07-22: Ticket lifecycle lock ownership checks are bounded to one second and fail safely for unreadable live owners. The unattended monitor reports only stuck Ticket helper/start-stop counts plus the oldest age and degrades with `pixel_ticket_lifecycle_stuck` after 60 seconds; an exact long-lived command reader remains detectable after reparenting to init, and process arguments are never persisted.

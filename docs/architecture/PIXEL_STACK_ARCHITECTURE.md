...
flowchart TD
  ...
  O --> P["Private pixel-orchestrator-observability SpacetimeDB module"]
```
...
- `fast`: the inner development lane. It defaults to orchestrator-only scope, reuses current APK and runtime artifacts when their content hashes match, performs local readiness checks, and records per-phase timings. A direct `redeploy_component ticket_screen` selects this profile when no profile was supplied, so the normal Ticket update path stays below its one-minute budget; other actions still default to `standard`. Timing telemetry contains only safe run metadata and is handed to the sibling deployment reporter asynchronously, so it cannot delay or change a deploy result. For `redeploy_component ticket_screen`, it keeps the mutation lock, target asset sync, runtime-input write, restart, and local `/api/v1/health` proof, but intentionally skips unrelated full-system/network probes; use `standard` or `full` when cross-component validation is required.
...
- 2026-07-13: General orchestrator events now use the private `pixel-orchestrator-observability` SpacetimeDB module with fixed safe fields, RAM-only 4 MiB/24-hour phone buffering, and database-time 24-hour deletion. Support bundles and cleanup summaries no longer retain raw logs or path lists.

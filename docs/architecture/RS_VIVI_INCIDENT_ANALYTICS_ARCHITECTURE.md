...
- 2026-07-10: Retired RS visual/pixel, accessibility semantic-loop, and shell-semantic drivers and their obsolete tests were removed after repository-wide caller checks showed the production path used only `RigasSatiksmeDirectTapDriver`. The active semantic classifier and bounded UI Automator XML parser were extracted as pure components and retain executable classification/parser coverage. Historical notes below describe the superseded rollout sequence, not code that remains available as a fallback.

- 2026-06-16: Pixel side of the admin-driven RS re-login channel is now implemented. `TicketStreamService` accepts `rigassatiksme_login_start` (with phone + locale), `rigassatiksme_login_sms` (with 4-8 digit code), and `cancel_rigassatiksme_login` on the existing `/api/v1/session` WebSocket. The new `RigasSatiksmeLoginOperation` is a pure state machine (`UNKNOWN → PHONE_FIELD → PHONE_ENTERED → SMS_FIELD → SMS_ENTERED → LOGIN_DONE | FAILED(phone_field_missing | sms_field_missing | wrong_sms_code | sms_timeout | submit_failed | phone_unavailable | login_unreachable | canceled)`) driven by `RigasSatiksmeLoginClassifier` against uiautomator snapshots. The Pixel types the phone via the same RS-only root-input shell used by the direct monthly-ticket driver, taps the sign-in button, waits for the broker to send the SMS code, types the SMS code, taps submit, and reports `rigassatiksme_login_result { state, failureReason, phoneLast4, phases }` back to the broker. Safety: the SMS code is held only in `RigassatiksmeLoginCodeHolder` (transient, cleared on consume/cancel/terminal) and is never logged, never written to disk, and never echoed in any `recordTicketEvent` or HTTP response. The phone is persisted only as `phoneLast4`. One SMS attempt per `start`; wrong code means a fresh `start`. Bounded: 12 actions or 90 s; ticket-priority preemptions cancel the login. Classification tests cover LV / EN / RU phone and SMS screens, the wrong-code surface, and the auth-blocked dialog, and assert that "Sign in"/"Log in" alone does not trigger the auth-blocked classifier.

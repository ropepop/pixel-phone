...
- Unified Ticket v3 actions use one action-scoped panel-dark lease rather than independent four-second non-touch windows. The lease is acquired inside the shared phone-mutation lane before ViVi launch and stays active through waits, navigation, registration, terminal visual proof, and the convergence tail. It writes raw panel zero every 25 ms and independently confirms zero every 250 ms. The runtime publishes its root-confirmed physical-touch state through the bridge; a physical touch stops the helper synchronously at the next mutation boundary or within the 20 ms monitor cadence. Before any phone dispatch the action stops safely; after a possible tap or held drag it returns attention-needed and cannot replay that attempt.
- Action lease cleanup is process-safe as well as coroutine-safe. Normal success, failure, cancellation, timeout, and service destruction release the helper in a non-cancellable finalizer. That finalizer first uses a separate root executor to validate the exact helper token, PID, process start time, marker, and owner arguments, then sends a bounded TERM/KILL sequence and proves both that the original process is gone and that its exact readiness file is absent. Only then does it perform a bounded coroutine join, so action completion cannot wait for the helper's 90-second maximum lifetime. The root helper still checks the exact Android owner PID and `/proc` start time on every write and self-terminates when that process disappears. After its first successful zero write, it creates a root-only per-action readiness file containing only process identities and start times. Acquisition reads that one file, validates the exact NUL-delimited helper arguments and live owner identity, and fails closed until two separate raw-zero confirmations agree; a pre-existing dark panel alone cannot authorize an action. Service startup scans only the capped readiness-file directory, kills exact validated helpers, removes invalid stale files, and refuses to subscribe to pending Ticket commands if cleanup fails.

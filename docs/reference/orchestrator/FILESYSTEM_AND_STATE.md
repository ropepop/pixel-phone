...
- `/data/local/pixel-stack/chroots/adguardhome/usr/local/bin/adguardhome-launch-frontend` - starts the remote frontend on public HTTPS/DoT listener ports (production contract now `443` and `853`)
...
- `/data/local/pixel-stack/chroots/adguardhome/etc/pixel-stack/remote-dns/state/querylog-view-preference.json` - shared saved default for native vs improved DNS query log view
- `/data/local/pixel-stack/chroots/adguardhome/etc/nginx/pixel-stack-adguardhome-remote-nginx.conf` - rendered nginx frontend config for the active public HTTPS/DoT listener ports (`443` and `853` in current production)
...
- `<app-data-dir>/shared_prefs/phone_automation_settings.xml` - persisted phone automation and touch brightness preferences, including durable Speedtest / CellMapper cycle metadata used to reconcile after restarts or short listener gaps

## Support Bundle Output

- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/health.json` - exported health snapshot
- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/config.json` - exported orchestrator config, redacted unless secrets are explicitly included
- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/state.json` - exported orchestrator state
- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/phone-automation-state.json` - exported phone automation snapshot, including durable run metadata, transient failure tracking, and current setup/runtime detail
- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/logs/*` - exported root runtime logs when readable
- `<app-external-files-dir>/Download/pixel-stack-support/bundle-<epoch-seconds>/run/*` - exported selected root runtime marker files (`*.pid`, `ddns-last-*`, `pihole-*`, `adguardhome-*`)

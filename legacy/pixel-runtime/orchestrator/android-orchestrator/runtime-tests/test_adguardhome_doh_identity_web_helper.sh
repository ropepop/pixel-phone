...
cat > "${ADGUARDHOME_DOH_IDENTITY_WEB_QUERYLOG_JSON_FILE}" <<'EOF_QUERYLOG'
{
  "data": [
    ...
    {"time":"__QUERYLOG_INTERNAL_BLOCKED_TIME__","client":"127.0.0.1","client_proto":"plain","elapsedMs":"6","status":"FILTERED","reason":"Blocked by filters","question":{"name":"blocked.internal.example","type":"A"},"client_info":{"whois":{}}}
  ]
}
EOF_QUERYLOG
python3 - <<'PY' \
...
if ! rg -Fq 'Download iPhone profile' "${identity_html}"; then
  echo "FAIL: identity page should expose the Apple profile download action" >&2
  exit 1
fi
...
setTimeout(() => {
  ...
  const runSetupGuideFlow = async () => {
    ...
    if (!(note.textContent || "").includes("https://example.test:2789/<doh-token>/dns-query")) {
      fail("setup guide note should show the tokenized public DoH URL");
    }
    if (!(note.textContent || "").includes("Do not use: https://example.test:2789/dns-query")) {
      fail("setup guide note should warn against the bare public DoH path");
    }
    if (!(note.textContent || "").includes("<identity-dot-hostname>.example.test:2790")) {
      fail("setup guide note should show the full public DoT target with port 2790");
    }
  ...
  const runQuerylogFlow = async () => {
    ...
    if (fetchLog.some((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"))) {
    ...
    if (!(querylogModal().textContent || "").includes("Blocked by filters") || !(querylogModal().textContent || "").includes("alpha-blocked.example.net")) {
      fail("querylog details popup should show the blocked-row details");
    }
    querylogModalCloseButton().dispatchEvent("click");
    await delay(20);
    ...
    dispatchHashchange();
    await delay(700);
    const searchRequests = fetchLog.filter((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"));
    ...
    dispatchHashchange();
    await delay(700);
    const noMoreSearchRequests = fetchLog.filter((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"));
    ...
    dispatchHashchange();
    await delay(700);
    const betaRequests = fetchLog.filter((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"));
    ...
    dispatchHashchange();
    await delay(700);
    const fallbackRequests = fetchLog.filter((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"));
    if (!querylogIdentitySelect() || querylogIdentitySelect().value !== "alpha") {
    ...
    dispatchHashchange();
    await delay(700);
    const autoLoadRequests = fetchLog.filter((entry) => entry.url.includes("/pixel-stack/identity/api/v1/adguard/querylog"));
...
if [[ ! -f "${ADGUARDHOME_DOH_IDENTITY_WEB_QUERYLOG_VIEW_PREFERENCE_FILE}" ]]; then
  echo "FAIL: querylog view preference API should write the persistent preference file" >&2
  exit 1
fi
if [[ "$(jq -r '.defaultView' "${ADGUARDHOME_DOH_IDENTITY_WEB_QUERYLOG_VIEW_PREFERENCE_FILE}")" != "improved" ]]; then
  echo "FAIL: querylog view preference file should store the improved default view" >&2
  exit 1
fi
...
if [[ "$(jq -r '.defaultView' "${ADGUARDHOME_DOH_IDENTITY_WEB_QUERYLOG_VIEW_PREFERENCE_FILE}")" != "native" ]]; then
  echo "FAIL: querylog view preference API should allow switching the saved default back to native" >&2
  exit 1
fi
...
if [[ "$(jq -r '.identities[0].dotTarget' "${list_after_create}")" != "${alpha_dot_hostname}:2790" ]]; then
  echo "FAIL: list endpoint should expose the full DoT target with port 2790" >&2
  exit 1
fi
if [[ "$(jq -r '.identities[0].dotTargetMasked' "${list_after_create}")" == "${alpha_dot_hostname}:2790" ]]; then
  echo "FAIL: dotTargetMasked should not expose the full DoT target" >&2
  exit 1
fi
if ! jq -r '.identities[0].dotTargetMasked' "${list_after_create}" | rg -Fq '.dns.jolkins.id.lv:2790'; then
  echo "FAIL: dotTargetMasked should preserve the shared DoT suffix and port" >&2
  exit 1
fi
...
expected_url = f"https://example.test:2789/{token}/dns-query"
if dns_settings.get("ServerURL") != expected_url:
  raise SystemExit("FAIL: Apple DoH profile should embed the full tokenized DoH URL on port 2789")
...
conn.execute(
    ...
    """,
    (row_time_ms, "alpha", "198.51.100.55", 200, 14, "burst-alpha.example.net", "A", "doh"),
)
conn.execute(
    """
    INSERT INTO doh_events (
    ...
    """,
    (row_time_ms, "beta", "198.51.100.55", 200, 14, "burst-beta.example.net", "A", "doh"),
)
conn.commit()
conn.close()
PY
...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=blocked&older_than=&limit=20" > "${proxy_querylog_blocked_json}"
...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=processed&older_than=&limit=20" > "${proxy_querylog_processed_json}"
...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=definitely_not_real&older_than=&limit=20" > "${proxy_querylog_invalid_status_json}"
...
if [[ "$(jq -r '.data | length' "${proxy_querylog_alpha_json}")" != "2" ]]; then
  echo "FAIL: identity-filtered proxy querylog should keep both matched alpha rows when the same client bursts on different names" >&2
  exit 1
fi
...
querylog_stale_seed_json="${tmpdir}/querylog-stale-seed.json"
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=all&older_than=&limit=20" > "${querylog_stale_seed_json}"
if [[ "$(jq -r '.pixelMeta.cacheState' "${querylog_stale_seed_json}")" != "fresh" ]]; then
...
querylog_stale_json="${tmpdir}/querylog-stale.json"
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=all&older_than=&limit=20" > "${querylog_stale_json}"
if [[ "$(jq -r '.pixelMeta.cacheState' "${querylog_stale_json}")" != "stale" ]]; then
  echo "FAIL: slow proxied querylog refresh should return stale cache metadata when fallback data exists" >&2
  exit 1
fi
if [[ "$(jq -r '.pixelMeta.stale' "${querylog_stale_json}")" != "true" ]]; then
  echo "FAIL: slow proxied querylog refresh should mark fallback data as stale" >&2
  exit 1
fi
if [[ "$(jq -r '.retryable' "${querylog_stale_json}")" != "true" ]]; then
  echo "FAIL: slow proxied querylog refresh should mark stale fallback payloads as retryable" >&2
  exit 1
fi
if ! jq -r '.error' "${querylog_stale_json}" | rg -Fq 'timed out'; then
  echo "FAIL: slow proxied querylog refresh should report a timeout error on stale fallback payloads" >&2
  exit 1
fi
if [[ "$(jq -r '.data | length' "${querylog_stale_json}")" == "0" ]]; then
  echo "FAIL: slow proxied querylog refresh should return the last successful row set instead of blank data" >&2
  exit 1
fi
clear_identityctl_delay
...
beta_dot_label="$(jq -r '.dotLabel' "${create_beta_json}")"
if [[ ! "${beta_token}" =~ ^[A-Za-z0-9._~-]{16,128}$ ]]; then
  echo "FAIL: recreated beta should return a valid generated token" >&2
  exit 1
fi
if [[ ! "${beta_dot_label}" =~ ^[a-z0-9]{20}$ ]]; then
  echo "FAIL: recreated beta should return a 20-char DoT label when DoT identities are enabled" >&2
  exit 1
fi
...
{"ts":1774389000,"tsMs":1774389000000,"identityId":"alpha","status":200,"requestTimeMs":10,"clientIp":"192.168.31.10"}
{"ts":1774389001,"tsMs":1774389001000,"identityId":"beta","status":404,"requestTimeMs":20,"clientIp":""}
EOF_MIGRATION_JSONL
env \
  ...
  "identities": [
    {
      ...
      "dotLabel": null,
      "createdEpochSeconds": 1774389000,
...
2026-03-24T21:00:00+00:00	/AlphaTokenAlphaToken1/dns-query?dns=a	200	0.010	192.168.31.20	1774395600.000
EOF_CURSOR_LOG_ONE
env \
...
2026-03-24T21:00:01+00:00	/AlphaTokenAlphaToken1/dns-query?dns=b	404	0.020	192.168.31.20	1774395601.000
EOF_CURSOR_LOG_TWO
env \
  ADGUARDHOME_DOH_IDENTITIES_FILE="${cursor_tmp}/doh-identities.json" \
  ADGUARDHOME_DOH_USAGE_EVENTS_FILE="${cursor_tmp}/state/doh-usage-events.jsonl" \
...
if [[ "$(jq -r '.totalRequests' "${cursor_tmp}/usage-second.json")" != "2" ]]; then
  echo "FAIL: identity helper should continue from the cursor without losing appended log rows" >&2
  exit 1
fi
if [[ "$(python3 -c "import sqlite3, sys; conn = sqlite3.connect(sys.argv[1]); print(conn.execute('select count(*) from doh_events').fetchone()[0]); conn.close()" "${cursor_tmp}/state/identity-observability.sqlite")" != "2" ]]; then
  echo "FAIL: cursor continuation should append exactly one new SQLite row without duplicates" >&2
  exit 1
fi
...
{"ts":1700000000,"tsMs":1700000000000,"identityId":"alpha","status":200,"requestTimeMs":10,"clientIp":"192.168.31.30"}
{"ts":1890000000,"tsMs":1890000000000,"identityId":"beta","status":200,"requestTimeMs":15,"clientIp":"192.168.31.31"}
EOF_PRUNE_JSONL

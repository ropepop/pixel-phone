...
ADB_DEVICES_FILE="${TMP_DIR}/adb-devices.txt"
...
if [[ "${1:-}" == "devices" ]]; then
  ...
  cat "${PIXEL_TEST_ADB_DEVICES_FILE}" 2>/dev/null || true
  printf '\n'
  exit 0
fi
...
case "${1:-}" in
  get-state)
    if [[ -n "${target_serial}" ]] && ! grep -Fq "${target_serial}" "${PIXEL_TEST_ADB_DEVICES_FILE}" 2>/dev/null; then
      exit 1
    fi
    printf 'device\n'
    ;;
  connect)
    connect_target="${2:-}"
    if [[ -n "${connect_target}" ]] && ! grep -Fq "${connect_target}" "${PIXEL_TEST_ADB_DEVICES_FILE}" 2>/dev/null; then
      printf '%s\tdevice\n' "${connect_target}" >> "${PIXEL_TEST_ADB_DEVICES_FILE}"
    fi
...
export PIXEL_TEST_ADB_DEVICES_FILE="${ADB_DEVICES_FILE}"
...
test_native_wireless_adb_is_preferred() {
  : > "${ADB_LOG}"
  cat > "${ADB_DEVICES_FILE}" <<'EOF'
100.64.0.10:5555	device
100.64.0.10:43463	device
EOF

  ADB_SERIAL=""
  local resolved=""
  resolved="$(pixel_transport_require_adb_serial)"
  [[ "${resolved}" == "100.64.0.10:43463" ]] || fail "expected native wireless adb target to be preferred"
}

test_native_wireless_adb_recovers_via_management_probe() {
  : > "${ADB_LOG}"
  : > "${ADB_DEVICES_FILE}"
  ADB_SERIAL=""
...
wireless_debug_tls_port=43463
wireless_debug_live=1
wireless_debug_healthy=1
...
EOF
  ...
  local resolved=""
  resolved="$(pixel_transport_require_adb_serial)"
  [[ "${resolved}" == "100.64.0.10:43463" ]] || fail "expected adb recovery to use the TLS connect port"
  assert_contains "${ADB_LOG}" "connect 100.64.0.10:43463"

  unset -f pixel_transport_fetch_management_probe
  unset PIXEL_TRANSPORT_SH_LOADED
  # shellcheck source=./transport.sh
  source "${TRANSPORT_SH}"
}

test_native_wireless_adb_recovers_from_legacy_probe_without_live_flag() {
  : > "${ADB_LOG}"
  : > "${ADB_DEVICES_FILE}"
  ADB_SERIAL=""
  PIXEL_TRANSPORT_MANAGEMENT_PROBE=""
...
EOF
  ...
  local resolved=""
  resolved="$(pixel_transport_require_adb_serial)"
  [[ "${resolved}" == "100.64.0.10:43463" ]] || fail "expected adb recovery to fall back to the legacy wireless debug flag"
  assert_contains "${ADB_LOG}" "connect 100.64.0.10:43463"

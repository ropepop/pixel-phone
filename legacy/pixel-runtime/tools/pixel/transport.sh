...
PIXEL_TRANSPORT_MANAGEMENT_PROBE="${PIXEL_TRANSPORT_MANAGEMENT_PROBE:-}"
ADB_BIN="${ADB_BIN:-adb}"
...
pixel_transport_resolve_adb() {
  ...
  local devices=()
  local resolved_serial=""

  pixel_transport_collect_connected_adb_devices devices
  if (( ${#devices[@]} > 0 )); then
    if resolved_serial="$(pixel_transport_select_connected_adb_serial "${devices[@]}")"; then
      ADB_SERIAL="${resolved_serial}"
      return 0
    fi
  fi

  if pixel_transport_try_native_wireless_adb_recovery >/dev/null 2>&1; then
    return 0
  fi
...
pixel_transport_connected_adb_devices() {
  "${ADB_BIN}" devices | awk 'NR>1 && $2=="device" {print $1}'
}

pixel_transport_collect_connected_adb_devices() {
  local ref_name="$1"
  local line=""
  eval "${ref_name}=()"
  while IFS= read -r line; do
    [[ -n "${line}" ]] || continue
    eval "${ref_name}+=(\"\${line}\")"
  done < <(pixel_transport_connected_adb_devices)
}

pixel_transport_serial_port() {
  local serial="$1"
  if [[ "${serial}" =~ :([0-9]+)$ ]]; then
    printf '%s\n' "${BASH_REMATCH[1]}"
  fi
...
pixel_transport_select_connected_adb_serial() {
  local -a direct_native=()
  local -a mdns_native=()
  local -a legacy=()
  local serial="" port=""
  ...
  for serial in "$@"; do
    ...
    port="$(pixel_transport_serial_port "${serial}")"
    if [[ -n "${port}" && "${port}" != "5555" ]]; then
      direct_native+=("${serial}")
      continue
    fi
    legacy+=("${serial}")
  done

  if (( ${#direct_native[@]} == 1 )); then
    printf '%s\n' "${direct_native[0]}"
    return 0
  fi
  if (( ${#direct_native[@]} > 1 )); then
    echo "Multiple native wireless adb devices found; pass --device" >&2
    return 1
  fi
  if (( ${#mdns_native[@]} == 1 )); then
    printf '%s\n' "${mdns_native[0]}"
    return 0
  fi
  if (( ${#mdns_native[@]} > 1 )); then
...
pixel_transport_probe_value() {
  local key="$1"
  local payload="${2:-${PIXEL_TRANSPORT_MANAGEMENT_PROBE:-}}"
...
pixel_transport_host_ssh_ready() {
  pixel_transport_fetch_management_probe >/dev/null
}

pixel_transport_fetch_management_probe() {
  if [[ -n "${PIXEL_TRANSPORT_MANAGEMENT_PROBE:-}" ]]; then
    printf '%s\n' "${PIXEL_TRANSPORT_MANAGEMENT_PROBE}"
    return 0
  fi
  ...
  PIXEL_TRANSPORT_MANAGEMENT_PROBE="${probe_output}"
  printf '%s\n' "${probe_output}"
}

pixel_transport_try_native_wireless_adb_recovery() {
  pixel_transport_require_cmd "${ADB_BIN}" || return 1

  local probe_output=""
  local wireless_debug_live=""
  local tailnet_ipv4=""
  local wireless_debug_tls_port=""
  local connect_target=""
  local devices=()
  local resolved_serial=""

  probe_output="$(pixel_transport_fetch_management_probe 2>/dev/null)" || return 1
  wireless_debug_live="$(pixel_transport_probe_value wireless_debug_live "${probe_output}")"
  if [[ -z "${wireless_debug_live}" ]]; then
    wireless_debug_live="$(pixel_transport_probe_value wireless_debug_healthy "${probe_output}")"
  fi
  tailnet_ipv4="$(pixel_transport_probe_value tailnet_ipv4 "${probe_output}")"
  wireless_debug_tls_port="$(pixel_transport_probe_value wireless_debug_tls_port "${probe_output}")"

  [[ "${wireless_debug_live}" == "1" ]] || return 1
  [[ -n "${tailnet_ipv4}" ]] || return 1
  [[ -n "${wireless_debug_tls_port}" ]] || return 1

  connect_target="${tailnet_ipv4}:${wireless_debug_tls_port}"
  "${ADB_BIN}" connect "${connect_target}" >/dev/null 2>&1 || true

  for _ in 1 2 3; do
    if "${ADB_BIN}" -s "${connect_target}" get-state >/dev/null 2>&1; then
      ADB_SERIAL="${connect_target}"
      return 0
    fi
    sleep 1
  done
...
pixel_transport_shell() {
  local command="$1"
  pixel_transport_require_device || return 1

  case "${PIXEL_TRANSPORT_RESOLVED}" in
    adb)
      "${ADB_BIN}" -s "${ADB_SERIAL}" shell "${command}"
      ;;
    ssh)
      pixel_transport_ssh_remote_shell "${command}"
      ;;
...
pixel_transport_exec() {
  local command=""
  command="$(pixel_transport_exec_join "$@")"
  pixel_transport_shell "${command}"
}

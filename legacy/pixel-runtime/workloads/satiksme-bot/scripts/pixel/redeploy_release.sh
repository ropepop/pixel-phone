...
prime_orchestrator_for_maintenance_window() {
  log "Priming orchestrator before Satiksme maintenance window"
  run_orchestrator \
    --action cleanup \
    --dry-run \
...
enter_satiksme_maintenance_window() {
  log "Stopping satiksme_bot before Spacetime schema publish"
  run_orchestrator \
...
recover_satiksme_maintenance_window() {
  if (( maintenance_window_active != 1 )); then
    return 0
  fi
  log "Restoring satiksme_bot after failed schema publish or redeploy"
  if ! run_orchestrator \
    --action start_component \
    --component satiksme_bot \
    --satiksme-bot-env-file "${REPO_ROOT}/.env"; then
    log "Automatic satiksme_bot recovery failed; manual intervention may be required"
  fi
}

clear_satiksme_maintenance_window() {
  maintenance_window_active=0
}

handle_redeploy_exit() {
  local rc=$?
  if (( rc != 0 )); then
    recover_satiksme_maintenance_window
  fi
...
trap handle_redeploy_exit EXIT

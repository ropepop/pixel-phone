...
origin_port="${SATIKSME_WEB_ORIGIN_PORT:-9327}"
...
asset_sha_from_body_if_present() {
  local version="$1"
  local url="$2"
  local target="$3"
  if [[ -z "${version}" ]]; then
  ...
  asset_sha_from_body "${url}" "${target}"
}
...
origin_live_client_body_sha=""
public_live_client_body_sha=""
parity_ok=0
...
for attempt in $(seq 1 "${attempts}"); do
  if curl -fsS --max-time 15 "$origin_health_url" -o "$tmp_dir/origin-health.json" &&
    ...
    origin_live_client="$(health_field "$tmp_dir/origin-health.json" "assets.liveClientSha256")"
    public_live_client="$(health_field "$tmp_dir/public-health.json" "assets.liveClientSha256")"
    if [[ -n "${origin_commit}" && "${origin_commit}" == "${public_commit}" &&
      ...
      -n "${origin_app_css}" && "${origin_app_css}" == "${public_app_css}" &&
      "${origin_live_client}" == "${public_live_client}" ]]; then
      ...
      origin_live_client_body_sha="$(asset_sha_from_body_if_present "${origin_live_client}" "${origin_base_url}/assets/live-client.js?v=${origin_live_client}" "$tmp_dir/origin-live-client.js")"
      public_live_client_body_sha="$(asset_sha_from_body_if_present "${public_live_client}" "${public_base_url%/}/assets/live-client.js?v=${public_live_client}" "$tmp_dir/public-live-client.js")"
      if [[ -n "${origin_app_js_body_sha}" && "${origin_app_js_body_sha}" == "${public_app_js_body_sha}" &&
        ...
        if [[ -z "${origin_live_client}" || "${origin_live_client_body_sha}" == "${public_live_client_body_sha}" ]]; then
          parity_ok=1
          break
        fi
      fi
...
if (( parity_ok == 1 )); then
  for _ in $(seq 1 10); do
    ...
    live_client_samples+=("$(header_value "$headers" "X-Satiksme-Bot-Live-Client")")
    sleep 1
  done
fi
...
unique_live_client_count="$(printf '%s\n' "${live_client_samples[@]:-}" | sort -u | wc -l | tr -d '[:space:]')"
headers_stable=0
if (( parity_ok == 1 )) &&
  [[ "${unique_instance_count:-0}" == "1" ]] &&
  [[ "${unique_app_js_count:-0}" == "1" ]] &&
  [[ "${unique_app_css_count:-0}" == "1" ]] &&
  [[ "${unique_live_client_count:-0}" == "1" ]] &&
  [[ "${instance_samples[0]:-}" == "${origin_instance}" ]] &&
  [[ "${app_js_samples[0]:-}" == "${origin_app_js}" ]] &&
  [[ "${app_css_samples[0]:-}" == "${origin_app_css}" ]] &&
  [[ "${live_client_samples[0]:-}" == "${origin_live_client}" ]]; then
  headers_stable=1
fi
...
export REPORT_ORIGIN_LIVE_CLIENT="${origin_live_client}"
export REPORT_PUBLIC_LIVE_CLIENT="${public_live_client}"
...
export REPORT_ORIGIN_LIVE_CLIENT_BODY_SHA="${origin_live_client_body_sha}"
export REPORT_PUBLIC_LIVE_CLIENT_BODY_SHA="${public_live_client_body_sha}"
...
export REPORT_LIVE_CLIENT_SAMPLES="$(IFS=,; printf '%s' "${live_client_samples[*]-}")"
...
report = {
    ...
    "origin": {
        ...
        "liveClientSha256": os.environ["REPORT_ORIGIN_LIVE_CLIENT"],
        "appJsBodySha256": os.environ["REPORT_ORIGIN_APP_JS_BODY_SHA"],
        "appCssBodySha256": os.environ["REPORT_ORIGIN_APP_CSS_BODY_SHA"],
        "liveClientBodySha256": os.environ["REPORT_ORIGIN_LIVE_CLIENT_BODY_SHA"],
    },
    "public": {
        ...
        "liveClientSha256": os.environ["REPORT_PUBLIC_LIVE_CLIENT"],
        "appJsBodySha256": os.environ["REPORT_PUBLIC_APP_JS_BODY_SHA"],
        "appCssBodySha256": os.environ["REPORT_PUBLIC_APP_CSS_BODY_SHA"],
        "liveClientBodySha256": os.environ["REPORT_PUBLIC_LIVE_CLIENT_BODY_SHA"],
        ...
        "liveClientSamples": [item for item in os.environ["REPORT_LIVE_CLIENT_SAMPLES"].split(",") if item],
    },
    "parity": {

...
browser_use_open_telegram_chat() {
  ...
  for ((attempt = 0; attempt < attempts; attempt++)); do
    ...
    if [[ -n "${result_ref}" ]]; then
      ...
      sleep 1
      browser_use_run_timed "${session}" 15 eval "(() => document.querySelector('#editable-message-text[aria-label=\"Message\"], .form-control.allow-selection[aria-label=\"Message\"]') ? 'messageReady=1' : 'messageReady=0')()" >/dev/null || true

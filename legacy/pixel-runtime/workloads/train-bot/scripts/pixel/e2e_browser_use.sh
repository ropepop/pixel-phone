...
session_name="${BROWSER_USE_BOT_SESSION:-${BROWSER_USE_SESSION:-$(browser_use_default_session_name "ttb")}}"
...
(async () => {
  ...
  const keyboardState = () => {
    ...
    return {
      ...
      openAppVisible: has(/Open app|Atvērt lietotni/i),
  ...
  let deepLinkVisible = false;

  for (let attempt = 0; attempt < 6; attempt++) {
    chatReady = await openBotChat();
    state = keyboardState();
    entry = checkinEntryState();
    deepLinkVisible = /https:\/\/t\.me\/vivi_kontrole_bot\/app/i.test(document.body ? document.body.innerText || '' : '');
    if (chatReady && (state.openAppVisible || deepLinkVisible)) {
      break;
    }
  ...
  state = keyboardState();
  return `chatReady=${chatReady ? 1 : 0};checkinVisible=${state.checkinVisible ? 1 : 0};reportVisible=${state.reportVisible ? 1 : 0};openAppVisible=${state.openAppVisible ? 1 : 0};deepLinkVisible=${deepLinkVisible ? 1 : 0};stationOptionVisible=${entry.stationOptionVisible ? 1 : 0};timeVisible=${entry.timeVisible ? 1 : 0};stationPromptVisible=${entry.stationPromptVisible ? 1 : 0}`;
})()
JS
)"

if ! browser_use_open_telegram_authenticated "$session_name" "$profile_spec" "$chat_url" 20; then
  fail "browser-use smoke failed during ${phase}: could not restore or open the Telegram session"
fi
if [[ "$mobile_view" == "1" ]]; then
  browser_use_run_timed "$session_name" 15 set viewport 390 844 >/dev/null 2>&1 || true
else
  browser_use_run_timed "$session_name" 15 set viewport 1280 1000 >/dev/null 2>&1 || true
fi
if ! browser_use_open_telegram_chat "$session_name" "8792187636" "@vivi_kontrole_bot" "Vivi kontrole bot" 4; then
  fail "browser-use smoke failed during ${phase}: could not open the Telegram bot chat"
fi
...
if ! browser_use_wait_for_eval_match "$session_name" "$js_bot_flow" 'chatReady=1.*(openAppVisible=1|deepLinkVisible=1)' 6 1; then
  fail "browser-use smoke failed during ${phase}: app entry was not visible through the launcher button or deep link"
fi

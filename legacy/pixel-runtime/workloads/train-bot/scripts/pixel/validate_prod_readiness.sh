...
train_web_public_base_url="${TRAIN_WEB_PUBLIC_BASE_URL:-https://train-bot.jolkins.id.lv}"
train_web_origin_port="${TRAIN_WEB_PORT:-9317}"
train_web_origin_forward_port="${TRAIN_WEB_ORIGIN_FORWARD_PORT:-19317}"
train_web_base_path="$(
  python3 - "${train_web_public_base_url}" <<'PY'
import sys
...
runtime_origin_health_file="$REPO_ROOT/output/pixel/runtime-origin-health-$timestamp_utc.json"
...
if (( ${#p1_findings[@]} == 0 )); then
  ...
  runtime_origin_health_url="http://127.0.0.1:${train_web_origin_forward_port}${train_web_base_path}/api/v1/health"
  runtime_origin_health_fallback_url="http://127.0.0.1:${train_web_origin_forward_port}/api/v1/health"
  adb_cmd forward --remove "tcp:${train_web_origin_forward_port}" >/dev/null 2>&1 || true
  adb_cmd forward "tcp:${train_web_origin_forward_port}" "tcp:${train_web_origin_port}" >/dev/null
  if curl -fsS --max-time 15 "${runtime_origin_health_url}" -o "${runtime_origin_health_file}" || curl -fsS --max-time 15 "${runtime_origin_health_fallback_url}" -o "${runtime_origin_health_file}"; then
    runtime_schedule_available="$(python3 - "${runtime_origin_health_file}" <<'PY'
import json
import sys
...
PY
    ...
    runtime_effective_service_date="$(python3 - "${runtime_origin_health_file}" <<'PY'
import json
import sys
...
PY
    ...
    runtime_fallback_active="$(python3 - "${runtime_origin_health_file}" <<'PY'
import json
import sys
...
PY
    ...
    if [[ "${runtime_schedule_available}" == "true" ]]; then
      status_schedule_file="PASS"
      status_schedule_load="PASS"
    else
      add_p1 "Runtime train health reported no active projection (expected ${service_date_riga}, effective=${runtime_effective_service_date:-unset}, fallback=${runtime_fallback_active:-false}). Details: ${runtime_origin_health_file}"
    fi
  else
    add_p1 "Runtime train health endpoint did not respond at ${runtime_origin_health_url} or ${runtime_origin_health_fallback_url}"
  fi
  adb_cmd forward --remove "tcp:${train_web_origin_forward_port}" >/dev/null 2>&1 || true

  runtime_spacetime_host="$(adb_cmd shell su -c "value=''; for env_file in /data/local/pixel-stack/apps/train-bot/env/train-bot.env /data/local/pixel-stack/apps/train-bot/env/config.env; do if [ -f \"\${env_file}\" ]; then value=\$(grep -E '^TRAIN_RUNTIME_SPACETIME_HOST=' \"\${env_file}\" | tail -n 1 | cut -d= -f2-); [ -n \"\$value\" ] && break; fi; done; printf '%s' \"\$value\"" | tr -d '\r' | sed -e "s/^['\"]//" -e "s/['\"]$//")"
  runtime_spacetime_database="$(adb_cmd shell su -c "value=''; for env_file in /data/local/pixel-stack/apps/train-bot/env/train-bot.env /data/local/pixel-stack/apps/train-bot/env/config.env; do if [ -f \"\${env_file}\" ]; then value=\$(grep -E '^TRAIN_RUNTIME_SPACETIME_DATABASE=' \"\${env_file}\" | tail -n 1 | cut -d= -f2-); [ -n \"\$value\" ] && break; fi; done; printf '%s' \"\$value\"" | tr -d '\r' | sed -e "s/^['\"]//" -e "s/['\"]$//")"

  if [[ -n "${runtime_spacetime_host}" && -n "${runtime_spacetime_database}" ]]; then
    status_schedule_db="PASS"
  else
    add_p1 "Runtime Spacetime host/database are missing (host=${runtime_spacetime_host:-unset}, db=${runtime_spacetime_database:-unset})"
  fi
...

{
  ...
  echo "- Runtime Spacetime config present: $status_schedule_db"

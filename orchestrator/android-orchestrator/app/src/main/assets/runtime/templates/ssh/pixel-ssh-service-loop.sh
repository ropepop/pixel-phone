...
find_other_loop_pid() {
  ...
  ps -A -o PID=,NAME=,ARGS= 2>/dev/null | awk -v target="${SELF_PATH}" -v self_pid="$$" '
    ...
    }
    {
      ...
      if (name == "pixel-ssh-service-loop" ||
        args == target ||
...
acquire_lock() {
  ...
  other_pid="$(find_other_loop_pid || true)"
  if pid_matches_loop "${other_pid}"; then
    echo "${other_pid}" > "${PID_FILE}"
    if [ ! -f "${DUPLICATE_MARK_FILE}" ]; then
      log "another ${LOOP_NAME} instance is already running (pid=${other_pid})"
      : > "${DUPLICATE_MARK_FILE}"
    fi
    return 1
  fi
...
release_lock() {
  current_pid="$(sed -n '1p' "${PID_FILE}" 2>/dev/null | tr -d '\r' || true)"
  if [ "${current_pid}" != "$$" ]; then
    return 0
  fi
  rm -f "${DUPLICATE_MARK_FILE}" >/dev/null 2>&1 || true

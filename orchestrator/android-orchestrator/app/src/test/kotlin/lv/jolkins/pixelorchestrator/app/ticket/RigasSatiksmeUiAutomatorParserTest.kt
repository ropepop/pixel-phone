...
class RigasSatiksmeUiAutomatorParserTest {
  @Test
  fun rejectsDoctypeAndEntityDeclarations() {
    assertThrows(IllegalArgumentException::class.java) {
      RigasSatiksmeUiAutomatorParser.parse(
        """<!DOCTYPE hierarchy [<!ENTITY secret SYSTEM "file:///data/local/private">]><hierarchy>&secret;</hierarchy>"""
      )
    }
  }
  ...
  fun parsesUiAutomatorControlTicketNodesFromRsApp() {
    val nodes = RigasSatiksmeUiAutomatorParser.parse(
      """
    ...
    assertEquals(
      ...
      RigasSatiksmeSemanticClassifier.classify(nodes, "68803")
    )
  ...
  fun parsesTripRegisteredModalAndOkButton() {
    ...
    assertEquals(
      ...
      RigasSatiksmeSemanticClassifier.classify(nodes, "58011")
    )
  ...
  fun classifiesLatvianRsEntryFlowStates() {
    assertEquals(
      ...
      RigasSatiksmeSemanticClassifier.classify(

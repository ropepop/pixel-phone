...
class TicketStreamServiceSourceTest {
  ...
  fun rootFfmpegSessionEnablesSecureWindowBypassBeforeLaunchingVivi() {
    ...
    val start = source.indexOf("activeCaptureMode = CAPTURE_MODE_ROOT_FFMPEG_H264")
  ...
  @Test
  fun ticketStartPrefersRootFfmpegAndDoesNotOpenProjectionPermission() {
    val source = ticketStreamServiceSource()
    ...
    assertTrue(block.contains("if (!ffmpegCaptureAvailable)"))
    assertTrue(block.contains("FFmpeg H.264 capture is unavailable; codec fallback was not started"))
    assertTrue(block.contains("activeCaptureMode = CAPTURE_MODE_ROOT_FFMPEG_H264"))
    assertTrue(block.contains("updateTicketSessionState(TICKET_SESSION_STARTING, \"session_start_root_ffmpeg_h264_prepare\")"))
    assertTrue(block.contains("scheduleRootPngCaptureStart(\"session_start_root_ffmpeg_h264_capture\""))
  ...
  fun projectionPrewarmIsNotUsedForNormalRootFfmpegStop() {
    ...
    assertTrue(startBlock.contains("activeCaptureMode = CAPTURE_MODE_ROOT_FFMPEG_H264"))
    assertTrue(startBlock.contains("scheduleRootPngCaptureStart(\"session_start_root_ffmpeg_h264_capture\""))

...
tmpdir="$(mktemp -d)"
sidecar_pid=""
trap 'echo DEBUG_TMPDIR="" >&2; if [[ -n "" ]] && kill -0 "" >/dev/null 2>&1; then kill "" >/dev/null 2>&1 || true; wait "" 2>/dev/null || true; fi' EXIT
...
if [[ "$(jq -r '.token' "${rename_beta_json}")" != "${beta_token}" ]]; then
  echo "DEBUG rename response:" >&2
  cat "" >&2
  echo "DEBUG expected token: " >&2

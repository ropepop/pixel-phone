...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=all&older_than=&limit=20" > "${querylog_fresh_seed_json}"
...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=all&older_than=&limit=20" > "${querylog_identityctl_json}"
...
curl -fsS "http://127.0.0.1:${port}/pixel-stack/identity/api/v1/adguard/querylog?search=&response_status=all&older_than=&limit=20" > "${querylog_unrelated_sql_json}"

...
DROPBEAR_PID_FILE="${TMP_DIR}/dropbear.pid"
...
/bin/sh "${BASE_DIR}/bin/pixel-ssh-service-loop" --with-dropbear "${BASE_DIR}/bin/dropbear" "${DROPBEAR_PID_FILE}" >/dev/null 2>&1 &
canonical_loop_pid="$!"

dropbear_pid=""
for _ in $(seq 1 20); do
  dropbear_pid="$(sed -n '1p' "${DROPBEAR_PID_FILE}" 2>/dev/null | tr -d '\r' || true)"
  if [[ "${dropbear_pid}" =~ ^[0-9]+$ ]] && kill -0 "${dropbear_pid}" >/dev/null 2>&1; then
    break
  fi
  sleep 0.5
done
...
if [[ ! "${dropbear_pid}" =~ ^[0-9]+$ ]]; then
  echo "FAIL: helper dropbear did not start" >&2
  exit 1
fi

cat > "${SS_OUTPUT_FILE}" <<EOF_LISTENER
LISTEN 0 128 0.0.0.0:2222 0.0.0.0:* users:(("dropbear",pid=${dropbear_pid},fd=3))
EOF_LISTENER
...
if [[ "${repaired_loop_pid}" != "${canonical_loop_pid}" ]]; then
  echo "FAIL: expected repaired loop pid ${canonical_loop_pid}, got ${repaired_loop_pid:-<empty>}" >&2
  exit 1
fi

if [[ ! -d "${BASE_DIR}/run/pixel-ssh-service-loop.lock" ]]; then
  echo "FAIL: reconcile did not recreate the loop lock directory" >&2
  exit 1
fi

if ! kill -0 "${canonical_loop_pid}" >/dev/null 2>&1; then
  echo "FAIL: reconcile replaced the healthy canonical loop" >&2
  exit 1
fi

if ! kill -0 "${dropbear_pid}" >/dev/null 2>&1; then
  echo "FAIL: reconcile interrupted the healthy dropbear listener" >&2
  exit 1
fi

loop_count="$(pgrep -f "${BASE_DIR}/bin/pixel-ssh-service-loop" | wc -l | tr -d ' ')"
if [[ "${loop_count}" != "1" ]]; then
  echo "FAIL: expected exactly one loop after reconcile, found ${loop_count}" >&2
  pgrep -fal "${BASE_DIR}/bin/pixel-ssh-service-loop" >&2 || true
  exit 1
fi

dropbear_start_lines="$(wc -l < "${DROPBEAR_LOG}" | tr -d ' ')"
if [[ "${dropbear_start_lines}" != "1" ]]; then
  echo "FAIL: expected reconcile to avoid relaunching dropbear, saw ${dropbear_start_lines} starts" >&2
  cat "${DROPBEAR_LOG}" >&2 || true
  exit 1
fi

echo "PASS: pixel-ssh-start repairs missing ownership state around a healthy live SSH runtime"

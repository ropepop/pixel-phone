...
| Codex Chrome integration | Yes | Required for browser smoke checks; use `chrome:control-chrome` first and fall back to legacy `browser-use` only if the Codex Chrome harness is unavailable or verified unsuitable. |
...
| miniapp smoke fails | browser session drift | rerun `make pixel-miniapp-smoke` after confirming Telegram Web auth in the Chrome session used for verification |
